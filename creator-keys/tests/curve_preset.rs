...
use creator_keys::{CurvePreset, CreatorKeysContract, CreatorKeysContractClient};

mod contract_test_env;
use contract_test_env::{
    register_test_creator_with_preset, set_key_price_for_tests, set_protocol_fee_bps,
    setup_env, DEFAULT_CREATOR_BPS, DEFAULT_PROTOCOL_BPS,
};
...
fn setup_with_fees() -> (Env, Address, CreatorKeysContractClient<'static>, Address) {
    ...
    // Set up fees
    set_protocol_fee_bps(&env, &client, DEFAULT_CREATOR_BPS, DEFAULT_PROTOCOL_BPS);
...
fn test_register_creator_with_quadratic() {
    ...
    client.register_creator(&creator, &handle, &Some(CurvePreset::Quadratic));
...
fn test_register_creator_with_flat() {
    ...
    client.register_creator(&creator, &handle, &Some(CurvePreset::Flat));
...
fn test_linear_preset_regression_matches_base_price() {
    ...
    let creator = register_test_creator_with_preset(&env, &client, "linear", CurvePreset::Linear);
    
    let quote = client.get_buy_quote(&creator);
    // At supply=0, Linear should match the base price
    assert_eq!(quote.price, creator_keys::bonding_curve::curve_params::BASE_PRICE);
}
...
fn test_quadratic_higher_than_linear_at_same_supply() {
    ...
    let linear_creator = register_test_creator_with_preset(&env, &client, "lin", CurvePreset::Linear);
    let quad_creator = register_test_creator_with_preset(&env, &client, "quad", CurvePreset::Quadratic);
    ...
    assert!(quad_quote.price > linear_quote.price,
        "quadratic price {} should exceed linear price {}", quad_quote.price, linear_quote.price);
}
...
fn test_flat_lower_than_linear_at_same_supply() {
    ...
    let linear_creator = register_test_creator_with_preset(&env, &client, "lin", CurvePreset::Linear);
    let flat_creator = register_test_creator_with_preset(&env, &client, "flat", CurvePreset::Flat);
    ...
    assert!(flat_quote.price < linear_quote.price,
        "flat price {} should be below linear price {}", flat_quote.price, linear_quote.price);
}
...
fn test_curve_preset_immutable_no_update_function() {
    ...
    let creator = register_test_creator_with_preset(&env, &client, "creator", CurvePreset::Linear);
...
fn test_independent_curves_no_cross_contamination() {
    ...
    let creator_a = register_test_creator_with_preset(&env, &client, "a", CurvePreset::Quadratic);
    let creator_b = register_test_creator_with_preset(&env, &client, "b", CurvePreset::Flat);
    ...
    assert!(quote_a.price > quote_b.price,
        "quadratic {} should exceed flat {}", quote_a.price, quote_b.price);
...
fn test_buy_sell_symmetry_all_presets() {
    for preset in [CurvePreset::Linear, CurvePreset::Quadratic, CurvePreset::Flat] {
        ...
        let creator = register_test_creator_with_preset(&env, &client, "sym", preset);
        let buyer = Address::generate(&env);
        ...
        assert_eq!(buy_quote.price, sell_quote.price,
...
fn test_creator_details_includes_preset() {
    ...
    let creator = register_test_creator_with_preset(&env, &client, "detailed", CurvePreset::Quadratic);
...
fn test_batch_view_includes_preset() {
    ...
    let creator_a = register_test_creator_with_preset(&env, &client, "a", CurvePreset::Linear);
    let creator_b = register_test_creator_with_preset(&env, &client, "b", CurvePreset::Flat);
