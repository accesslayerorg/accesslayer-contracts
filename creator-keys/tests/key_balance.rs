...
fn test_key_balance_is_per_creator() {
    ...
    client.set_key_price(&admin, &100i128);
    client.register_creator(&creator_a, &String::from_str(&env, "alice"), &None);
    client.register_creator(&creator_b, &String::from_str(&env, "bob"), &None);
...
fn test_key_balance_zero_for_unregistered_creator_even_when_other_balances_exist() {
    ...
    client.set_key_price(&admin, &100i128);
    client.register_creator(&registered_creator, &String::from_str(&env, "alice"), &None);
