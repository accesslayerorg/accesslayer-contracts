...
fn test_buy_keys_within_limit() {
    ...
    let result = client.try_buy_keys(&creator, &buyer, &2, &total_payment, &None);
...
fn test_buy_keys_exceeds_limit() {
    ...
    let result = client.try_buy_keys(&creator, &buyer, &3, &total_payment, &None);
...
fn test_buy_keys_no_limit_set() {
    ...
    let result = client.try_buy_keys(&creator, &buyer, &5, &total_payment, &None);
...
fn test_buy_keys_zero_quantity_fails() {
    ...
    let result = client.try_buy_keys(&creator, &buyer, &0, &100, &None);
...
fn test_buy_keys_emits_event_on_set() {
    ...
    assert!(
        has_event,
        "max_buy_quantity_updated event should be emitted"
    );
}
