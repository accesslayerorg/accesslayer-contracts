...
fn bumps_ttl_of_every_read_config_entry() {
    ...
    // Fast-forward far enough that every read config entry is near expiry.
    let mut ledger = env.ledger().get();
    ledger.sequence_number += creator_keys::CREATOR_TTL_LEDGERS - 100;
    env.ledger().set(ledger);

    let mut ttl_before = std::vec::Vec::new();
    for k in keys.iter() {
    ...
    for (k, before) in keys.iter().zip(ttl_before.iter()) {
        ...
        assert!(
            after >= creator_keys::TTL_MIN_EXTENSION_LEDGERS,
            "expected get_protocol_status to refresh TTL of {k:?} to at least 30 days (after={after})"
        );
    }
}
