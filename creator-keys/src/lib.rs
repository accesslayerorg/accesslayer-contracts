...
pub enum DataKey {
    ...
    /// Configured pre-launch auction state for a creator.
...
///
/// While an auction is configured, keys sell at the fixed `auction_price`
...
impl CreatorKeysContract {
    ...
    pub fn sell_key(
        ...
        if liquid_balance == 0 {
            if staked_balance > 0 {
                return Err(ContractError::KeysStaked);
            }
            return Err(ContractError::InsufficientBalance);
        }
        ...
        if let Some(created_at) = env
            .storage()
            .persistent()
            .get::<DataKey, u32>(&created_at_key)
        {
            // The launch-window check reads this key on every sell; keep it
            // well clear of expiry so an archived entry can never break sells.
            extend_key_ttl_to_full_window(&env, &created_at_key);
            let current_ledger = env.ledger().sequence();
                ...
                < crate::LAUNCH_PENALTY_WINDOW_LEDGERS
            {
                ...
                if capped_bps > 0 {
                    ...
                    if penalty_amount > 0 {
                        // The launch penalty is not a protocol fee: it is
                        // credited to the staking rewards pool in full (no
                        // rewards-share scaling) to reward committed holders.
                        let pool_key = constants::storage::staking_rewards_pool(&creator);
                        let mut state: StakingRewardsState =
                            env.storage().persistent().get(&pool_key).unwrap_or(
                                StakingRewardsState {
                                    pool: 0,
                                    total_staked: 0,
                                },
    ...
    /// [`ContractError::KeysStaked`] is returned. Once configured,
    ...
    pub fn stake_keys(
        ...
        // Lock the keys into a staking position for the standard 30-day
        // window so the holder can claim a pro-rata reward via
        // `claim_stake_reward` once the position matures.
        let stake_id =
            Self::next_stake_id(&env, &creator, &holder).map_err(|_| ContractError::Overflow)?;
    ...
    pub fn unstake_keys(
        ...
        // Decrement the cross-holder staked total so reward distribution
        // accounting stays consistent with the released keys.
        let pool_key = constants::storage::staking_rewards_pool(&creator);
        let mut state: StakingRewardsState =
            env.storage()
                .persistent()
    ...
    pub fn claim_stake_reward(
        ...
        if env.ledger().sequence() < position.unlock_ledger {
            return Err(StakingError::StakeLockActive);
        }
