...
pub mod constants {
  ...
  pub mod storage {
    ...
    pub const PROTOCOL_FEE_RECIPIENT_BALANCE: DataKey =
      DataKey::ProtocolFeeRecipientBalance;
...
pub fn read_creator_fee_recipient_balance(
  env: &Env,
  creator: &Address,
) -> i128 {
...
fn read_required_protocol_fee_config(
  env: &Env,
) -> Result<fee::FeeConfig, ContractError> {
...
fn credit_protocol_fee_recipient_balance(
  env: &Env,
  amount: i128,
) -> Result<(), ContractError> {
  if amount <= 0 {
...
impl CreatorKeysContract {
  ...
  pub fn distribute_dividend(
    ...
    let native_token =
      soroban_sdk::token::TokenClient::new(&env, &native_address);
    ...
    for holder in holders.iter() {
      let balance =
        Self::get_key_balance(env.clone(), creator.clone(), holder.clone())
          as i128;
      if balance == 0 {
        continue;
      }
      ...
      if share > 0 {
        ...
        let new_claim =
          current.checked_add(share).ok_or(ContractError::Overflow)?;
  ...
  pub fn get_creator_fee_recipient(
    env: Env,
    creator: Address,
  ...
  pub fn get_creator_fee_balance(
    env: Env,
    creator: Address,
  ) -> Result<i128, ContractError> {
    read_registered_creator_profile(&env, &creator)?;
  ...
  pub fn get_creator_treasury_share(
    env: Env,
    creator: Address,
  ) -> Result<u32, ContractError> {
  ...
  pub fn get_protocol_treasury_share_bps(
    env: Env,
  ) -> Result<u32, ContractError> {
    let config = read_required_protocol_fee_config(&env)?;
  ...
  pub fn compute_fees_for_payment(
    env: Env,
    total: i128,
  ) -> Result<(i128, i128), ContractError> {
    let config = read_required_protocol_fee_config(&env)?;
    fee::checked_compute_fee_split(
      total,
      config.creator_bps,
      config.protocol_bps,
    )
  ...
  pub fn get_buy_quote(
    ...
    let (creator_fee, protocol_fee) =
      Self::compute_fees_for_payment(env.clone(), price)?;
...
mod tests {
  ...
  fn test_checked_format_quote_response_sell_success() {
    let res =
      super::checked_format_quote_response(1000, 90, 10, false).unwrap();
    assert_eq!(res.price, 1000);
    assert_eq!(res.creator_fee, 90);
    assert_eq!(res.protocol_fee, 10);
