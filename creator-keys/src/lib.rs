...
/// Extends the TTL of a persistent storage entry only when it already exists.
///
/// Unlike [`bump_persistent_ttl`], this never panics on an unconfigured key:
/// Soroban's `extend_ttl` rejects a key that is not present in storage with
/// `Error(Storage, MissingValue)`, so read-only views that bump optional config
/// entries must check existence first.
fn bump_persistent_ttl_if_present(env: &Env, key: &DataKey) {
...
impl CreatorKeysContract {
    ...
    pub fn get_protocol_status(env: Env) -> ProtocolStatus {
        ...
        bump_persistent_ttl_if_present(&env, &constants::storage::GLOBAL_TRADING_PAUSED);
        ...
        let protocol_fee_bps: u32 = env
            ...
            .get(&constants::storage::PROTOCOL_FEE_BPS)
            .unwrap_or(0u32);
        bump_persistent_ttl_if_present(&env, &constants::storage::PROTOCOL_FEE_BPS);

        let treasury_address: Option<Address> = env
            .storage()
            .persistent()
            .get(&constants::storage::TREASURY_ADDRESS);
        bump_persistent_ttl_if_present(&env, &constants::storage::TREASURY_ADDRESS);
        ...
        bump_persistent_ttl_if_present(&env, &constants::storage::LOCKUP_DURATION_SECS);

        let min_investment_amount: Option<i128> = env
            .storage()
            .persistent()
            .get(&constants::storage::MIN_INVESTMENT_AMOUNT);
        bump_persistent_ttl_if_present(&env, &constants::storage::MIN_INVESTMENT_AMOUNT);
