...
impl CreatorKeysContract {
    ...
    pub fn buy_key(
        ...
        if current_balance == 0 {
            profile.holder_count = profile
                ...
                .checked_add(1)
                .ok_or(ContractError::Overflow)?;
            // Add the buyer to the holders list
    ...
    pub fn sell_key(
        ...
        if new_balance == 0 {
            profile.holder_count = profile
                ...
                .checked_sub(1)
                .ok_or(ContractError::SellUnderflow)?;
            // Remove the seller from the holders list
            let mut holders = read_holders_list(&env, &creator);
    ...
    /// Distributes a dividend to all current key holders of a creator.
    ///
    /// Anyone can call this function. The caller must transfer `amount` XLM to the contract.
    /// The amount is split proportionally among all holders based on their key balance at
    /// the moment of the call. A protocol fee is deducted before splitting.
    ///
    /// Emits a `DividendDistributed` event.
    pub fn distribute_dividend(
        env: Env,
        creator: Address,
        amount: i128,
    ) -> Result<(), ContractError> {
        let caller = env.caller(); // use caller() for compatibility with older SDK

        // Hardcode the Stellar native token address (same on all networks)
