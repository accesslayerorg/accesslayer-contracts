...
fn compute_linear_price(_supply: u32, amount: u32) -> Option<i128> {
    let n = amount as i128;
    // Linear preset: constant price per key (backward-compatible with old fixed-price system)
    BASE_PRICE.checked_mul(n)
}
        ...
        for preset in [
            ...
            for supply in [0u32, 1, 5, 10] {
                for amount in [1u32, 2, 5] {
                    ...
                    // After buying, supply becomes supply + amount
                    // Selling `amount` keys at supply + amount should return
                    // the price of the keys bought at supply..supply+amount-1
                    let sell_supply = supply;
                    let sell_price = compute_price(sell_supply, amount, preset).unwrap();
                    assert_eq!(
